...
import com.offcn.user.service.ProvincesService;
...
public class ProvincesController {
    ...
    public Result<PageResult<Provinces>> findPage(@RequestBody(required = false) @ApiParam(name = "Provinces对象",value = "传入JSON数据",required = false) Provinces provinces, @PathVariable  int page, @PathVariable  int size){
        //调用ProvincesService实现分页条件查询Provinces
        PageResult<Provinces> pageResult = provincesService.findPage(provinces, page, size);
    ...
    public Result<PageResult<Provinces>> findPage(@PathVariable  int page, @PathVariable  int size){
        //调用ProvincesService实现分页查询Provinces
        PageResult<Provinces> pageResult = provincesService.findPage(page, size);
        return new Result<PageResult<Provinces>>(true,StatusCode.OK,"查询成功",pageResult);
    }
    ...
    @ApiImplicitParam(paramType = "path", name = "id", value = "主键ID", required = true, dataType = "Integer")
    @PutMapping(value="/{id}")
    public Result update(@RequestBody @ApiParam(name = "Provinces对象",value = "传入JSON数据",required = false) Provinces provinces,@PathVariable Integer id){
        //设置主键值
        provinces.setId(id);
