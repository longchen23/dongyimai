...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    public PageResult<User> findPage(int page, int size) {
        Page<User> mypage = new Page<>(page, size);
        IPage<User> iPage = this.page(mypage, new QueryWrapper<>());
     ...
     * @return points
     */
    //@GlobalTransactional(name = "add",rollbackFor = Exception.class)
    @Transactional(propagation = Propagation.REQUIRES_NEW)
    @Override
    public int addUserPoints(String username, Integer points) {
        String xID = RootContext.getXID();
        System.out.println("user微服务全局事务ID: " + xID);
        System.out.println("=========");
        int count = userMapper.addUserPoints(username, points);

        System.out.println("添加积分受影响行数: " + count);
        //int i=10/0;
        return count;
    }
